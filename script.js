...
// 1. DATOS DE PERSONAJES (7 PERSONAJES CORREGIDOS)
const PERSONAJES = {
    "Viajero": { 
...
document.querySelectorAll('.char-circle').forEach(boton => {
    boton.addEventListener('click', () => {
        ...
        if (data) {
            ...
            // Cambio de color suave según el personaje
            modalContent.style.backgroundColor = data.color;
            modalContent.style.borderColor = data.color;
            modalContent.style.boxShadow = `0 0 30px ${data.color}`;
...
function moveSlide(n) {
    ...
    if (slides.length === 0) return; // Seguridad si no hay imágenes
